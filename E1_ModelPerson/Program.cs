...
namespace Ex1_Person
{
    ...
    static class Program
    {
        ...
        // This is done by the app to Initialise Database File using Cmd Prompt
        ...
        static void SetupLocalDb(string InstanceName = "mssqllocaldb", string DatabaseName = "TestDb") //mssqllocaldb is a default instance name in local DB v13
        {
            ...
            if (!CheckDatabaseExists(connectionString, DatabaseName))
            {
                ...
                string AppDataPath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\HomeOpen";
                ...
                using (connection)
                {
                    ...
                    MessageBox.Show($" A New empty Local Database has been created successfully!\r\n\r\nInstance: sqllocaldb\\({InstanceName})\r\nDatabase: {DatabaseName}");
                }
            }

            //Step3: Check EF works
            using (PersonContext db = new PersonContext())
            {
                // As soon as you create db context and database does not exist it will error
        ...
        static void Main()
        {
            //Note: Instance and Database to match connection string in EF Visual Designer 
            // e.g. 

            SetupLocalDb("dbinstance", "People");
